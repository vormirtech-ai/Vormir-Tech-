...
Migrations run in order and are tracked with SQLite's `user_version`. Never
edit one that has shipped — append a new one instead. This schema is identical
to the JavaScript edition's, so a .db file opens in either.
"""
...
MIGRATIONS = [
    {"version": 1, "name": "initial-schema", "sql": INITIAL_SCHEMA},
]
