...
> **Starting MedV:** double-click `start-medv.bat` (or `./start-medv.sh`). Your
> browser opens on the sign-in screen. Leave the small black window open while
> you work — closing it stops MedV. Your data is saved continuously either way.
...
1. **Backup → Backup to…**
2. MedV writes the file into `Documents\MedV Backups\` and shows you the exact
   path — click **Show in folder** to open it.
3. Copy that file onto a pen drive. Keep a week of them.
...
* **MedV will not start** — another copy is probably already running; close its
  window and try again. If the browser says "MedV is not responding", the black
  window was closed — start it again.
* **"Python was not found"** — install Python from python.org and tick
