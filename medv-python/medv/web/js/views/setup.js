  ...
  function draw() {
    ...
    mount(host,
      ...
      el('div.note.mt-16', icon('shield'), el('div',
        el('b', 'Everything stays on this computer. '),
        window.medv && window.medv.isElectron
          ? `The database file is created at ${store.state?.paths?.db || 'your application data folder'}.`
          : 'Your database is created inside this browser, on this computer — nothing is uploaded. '
            + 'Download a backup from the Backup screen regularly and keep it on a pen drive.')));
  }
